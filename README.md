<head>
  ...
  body>:not(header):not(footer):not(.hero){
  ...
  .project-block{
    ...
    transition:box-shadow .3s,border-color .3s,transform .05s;
    margin:22px 0;
    padding:16px 18px;
    position:relative;
    overflow:hidden;
  }
  .project-block:hover, .project-block:focus-within{
    border-color:rgba(0,255,240,.35);
    box-shadow:0 0 20px rgba(0,255,240,.25),0 0 40px rgba(0,255,240,.15),0 6px 24px rgba(0,0,0,.45);
  }
  .project-block:active{transform:translateY(1px)}
  .project-block p>strong:first-child{display:inline;margin:0 4px 0 0;white-space:nowrap}
  ...
  /* ===== Coverage Map — EXACT Metrics look ===== */
  /* Wrap the table in: <div id="coverage-map" class="table-wrap">…</div> */
  #coverage-map{ overflow:auto; border-radius:8px; }
  ...
  /* flat rows, thin separators, no zebra/hover */
  #coverage-map tbody th,
  #coverage-map tbody td{
    background:var(--panel);
    color:var(--text);
    padding:12px 14px;
    vertical-align:top;
